...
public class MediaAssetService implements IMediaAssetService {
    private static final Set<String> ALLOWED_MIME_TYPES = Set.of("image/jpeg", "image/png", "image/webp", "application/pdf");
    ...
    public UploadUrlResponse createUploadUrl(UploadUrlRequest request) {
        ...
        var storageKey = "media/%s/%s-%s".formatted(
                request.usage() == null || request.usage().isBlank() ? "general" : slug(request.usage()),
                UUID.randomUUID(),
                slug(request.originalFilename()));
        var target = storageService.createUploadTarget(storageKey, request.mimeType(), request.sizeBytes());
    ...
    public MediaAsset confirm(Long id, ConfirmUploadRequest request) {
        var asset = repository.findById(id).orElseThrow(() -> new NotFoundException("Mídia não encontrada"));
    ...
    private boolean hasActiveReference(Long id) {
        var sql =
                ...
                """;
        return Boolean.TRUE.equals(jdbcTemplate.queryForObject(sql, Boolean.class, id, id, id, id, id, id));
    }

    private String publicUrl(String storageKey) {
        var base = seoSettingRepository.findAll().stream()
                .findFirst()
